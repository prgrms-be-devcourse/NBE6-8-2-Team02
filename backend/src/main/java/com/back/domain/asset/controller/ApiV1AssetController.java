...
public class ApiV1AssetController {
    ...
    public ResponseEntity<RsData<AssetDto>> createAsset(
        ...
        return ResponseEntity
                ...
                .body(new RsData<>("200-1", "자산이 등록되었습니다.", assetDto));
    }

    // 다건 조회
    @GetMapping
    @Operation(summary = "자산 다건 조회")
    public ResponseEntity<RsData<List<AssetDto>>> getAssets() {
        List<Asset> assets = assetService.findAll();
        List<AssetDto> assetDtos = assets.stream().map(AssetDto::new).toList();
        return ResponseEntity
                .status(HttpStatus.OK)
                .body(new RsData<>("200-1", "자산 목록을 조회했습니다.", assetDtos));
    }

    // 단건 조회
    @GetMapping("/{id}")
    @Operation(summary = "자산 단건 조회")
    public ResponseEntity<RsData<AssetDto>> getAsset(@PathVariable int id) {
        Asset asset = assetService.findById(id).orElse(null);
        AssetDto assetDto = new AssetDto(asset);
        return ResponseEntity
                .status(HttpStatus.OK)
                .body(new RsData<>("200-1", "%d번 자산을 조회했습니다.".formatted(id), assetDto));
    }

    // id 기반 삭제
    @DeleteMapping("/{id}")
    @Operation(summary = "자산 삭제 (id 기반)")
    public ResponseEntity<RsData<AssetDto>> deleteAsset(@PathVariable int id) {
        Asset asset = assetService.deleteById(id);
        AssetDto assetDto = new AssetDto(asset);
        return ResponseEntity
                .status(HttpStatus.NO_CONTENT)
                .body(new RsData<>("200-1", "%d번 자산을 삭제했습니다.".formatted(id), assetDto));
    }

    // id 기반 수정
    @PutMapping("/{id}")
    @Operation(summary = "자산 수정 (id 기반)")
    public ResponseEntity<RsData<AssetDto>> updateAsset(@RequestBody UpdateAssetRequestDto updateAssetRequestDto) {
        Asset asset = assetService.updateById(updateAssetRequestDto);
        AssetDto assetDto = new AssetDto(asset);
        return ResponseEntity
                .status(HttpStatus.OK)
                .body(new RsData<>("200-1", "%d번 자산을 수정했습니다.".formatted(updateAssetRequestDto.id()), assetDto));
    }

    @GetMapping("/member")
    @Operation(summary = "사용자 기반 자산 목록 조회")
    public ResponseEntity<RsData<List<AssetDto>>> getAssetsByCurrentMember(@AuthenticationPrincipal CustomUserDetails userDetails) {
        ...
        return ResponseEntity
                .status(HttpStatus.OK)
                .body(new RsData<>("200-1", "%d번 사용자의 자산 목록을 조회했습니다.".formatted(memberId), assetDtos));
    }

    @PostMapping("/member")
    @Operation(summary = "사용자 기반 자산 등록")
    public ResponseEntity<RsData<AssetDto>> createAssetByCurrentMember(
